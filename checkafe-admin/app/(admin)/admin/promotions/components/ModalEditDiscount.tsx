...
import { Discount, DiscountForm } from "../types"
...
export default function ModalEditDiscount({ open, onClose, discount, onSuccess }: ModalEditDiscountProps) {
  ...
  const onSubmit = async (data: DiscountForm) => {
    if (!discount) return
    try {
      await authorizedAxiosInstance.put(`/v1/discounts/${discount._id}`, data)
      toast.success("Cập nhật ưu đãi thành công!")
      onClose()
      onSuccess?.()
    } catch (err: any) {
      toast.error(err?.response?.data?.message || "Cập nhật ưu đãi thất bại")
    }
  }
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block font-medium">Shop ID <span className="text-red-500">*</span></label>
            <Input {...register("shop_id", { required: true })} placeholder="ID của cửa hàng áp dụng" />
            <p className="text-xs text-gray-500 mt-1">ID của cửa hàng áp dụng ưu đãi (lấy từ hệ thống).</p>
          </div>
